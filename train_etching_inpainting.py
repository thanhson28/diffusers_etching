...
class EtchingDataset(Dataset):
    def __init__(self, root_dir, prompt_file):
        data = []
        with open(prompt_file, 'rt') as f:
            for line in f:
                data.append(json.loads(line))
        self.data = data
        keywords = ["Step", "Time", "S Pwr", "B2 Pwr", "Gas A", "Gas E", "Temp 1", "Temp 2", "Temp 3", "Temp 4", "Temp 5", "Temp 6", "Temp 7"]
    ...
    def __getitem__(self, idx):
        ...
        prompt = item['prompt']
        # read source by PIL and convert to RGB
        source = PIL.Image.open('/home/compu/son/ControlNet/training/etching/' + source_filename).convert('RGB')
        source = PIL.Image.new('RGB', source.size, (255, 255, 255))
        target = PIL.Image.open('/home/compu/son/ControlNet/training/etching/' + target_filename).convert('RGB')
        ...
        if h > w:
            pad = (h - w) // 2
            # padding by PIL
            source = PIL.ImageOps.expand(source, border=(pad, 0, pad, 0), fill=(0, 0, 0))
            target = PIL.ImageOps.expand(target, border=(pad, 0, pad, 0), fill=(0, 0, 0))
        elif w > h:
            pad = (w - h) // 2
            source = PIL.ImageOps.expand(source, border=(pad, 0, pad, 0), fill=(0, 0, 0))
            target = PIL.ImageOps.expand(target, border=(pad, 0, pad, 0), fill=(0, 0, 0)) 
        
        # pair elements in prompt with keywords and convert to string
        # example: "Step is 200, Time is 3000, S Pwr is 3000, B2 Pwr is 156, Gas A is 934, Gas E is 20, Temp 1 is 20, Temp 2 is 15, Temp 3 is 15, Temp 4 is 15, Temp 5 is 15, Temp 6 is 15, Temp 7 is 15"
        prompt_dict = dict(zip(self.keywords, prompt.split(',')))
        #shuffle the prompt dict
        prompt_dict = {k: v for k, v in sorted(prompt_dict.items(), key=lambda item: np.random.random())}
        prompt = ','.join([f"{k} {v}" for k, v in prompt_dict.items()])

        #save test image
        target.save('test.jpg')
        
        return {"prompt": prompt, "image": target, "mask_0": source}
...
class DreamBoothDataset(Dataset):
    def __init__(
        ...
        self.train_data = EtchingDataset('/home/compu/son/ControlNet/training/etching/', '/home/compu/son/ControlNet/training/etching/prompt.json')
...
def log_validation(pipeline, text_encoder, unet, val_pairs, accelerator):
    ...
    with torch.autocast("cuda"):
        # shuffle the validation pairs
        random.shuffle(val_pairs)
        sub_val_pairs = val_pairs[:4]
        val_results = [{"data_or_path": pipeline(**pair).images[0], "caption": pair["prompt"]} for pair in sub_val_pairs]
    #concate original image, prompt, mask and generated image and log to wandb
    for val_result, pair in zip(val_results, sub_val_pairs):
        original_image = pair["image"]
        prompt = pair["prompt"]
        mask = pair["mask_image"]

        generated_image = val_result["data_or_path"]

        # concatenate the images and prompt into one image
        concat_image = Image.new('RGB', (original_image.width * 4, original_image.height))
        concat_image.paste(original_image, (0, 0))
        
        from PIL import ImageDraw, ImageFont
        prompt_image = Image.new('RGB', (original_image.width, original_image.height), color='white')
        draw = ImageDraw.Draw(prompt_image)
        font = ImageFont.load_default()
        #check if the prompt is too long, if so, split it into multiple lines
        if len(prompt) > 50:
            prompt = [prompt[i:i+50] for i in range(0, len(prompt), 50)]
            prompt = '\n'.join(prompt)
        draw.multiline_text((10, 10), prompt, fill='black', font=font, spacing=4, font_size=50)
        concat_image.paste(prompt_image, (original_image.width*2, 0))

        concat_image.paste(mask.convert("RGB"), (original_image.width * 3, 0))
        concat_image.paste(generated_image, (original_image.width, 0))

        # create white image with prompt black text

        val_result["data_or_path"] = concat_image
...
def main():
    ...
    # val_pipeline.scheduler = DDIMScheduler.from_config(val_pipeline.scheduler.config)
    # import ipdb; ipdb.set_trace()
    val_pipeline.set_progress_bar_config(disable=False)
